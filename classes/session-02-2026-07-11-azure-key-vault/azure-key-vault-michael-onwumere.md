...
•	Hardcoding secrets (API keys, passwords, connection strings) directly in application config files or source code is a dangerous security anti-pattern. A single exposed secret can escalate into full workload compromise through a predictable attack chain:
...
3.	Call Key Vault — App presents the token as Bearer auth.
4.	Authorize & Return — RBAC is checked; secret is returned over TLS.
